function [phi] = LIV_CV_CP(sigma_phi,num_inter,sigma,file_name,mu,a_1,timestep,epsilon)
...
K = fspecial('gaussian',round(sigma*(4*sqrt(2*log(2)))));
K_phi = fspecial('gaussian',round(sigma_phi*(4*sqrt(2*log(2)))));
I=Img(:,:,1);
...
force_image = 0; %
c1 = sum(sum(sum(L.*Heaviside(phi,epsilon))))/(length(inidx)+eps); % average inside of Phi0
c2 = sum(sum(sum(L.*(1-Heaviside(phi,epsilon)))))/(length(outidx)+eps); % verage outside of Phi0
...
GIF=force;
LIF=((I - f1.*Hphi - f2.*(1 - Hphi)).*(f1 - f2));
phi = phi + timestep*(DiracPhi.*((a_1.*LIF)+(1-a_1)*GIF));
end
